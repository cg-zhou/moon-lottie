...
export const supportSections = [
  ...
  },
  {
    ...
    rows: [
      ...
      row("相交", officialMaskIntersect, maybe("未系统验证相交蒙版模式")),
      row("变亮", officialAllUnsupported, no),
      row("变暗", officialAllUnsupported, no),
      row("差值", officialAllUnsupported, no),
      row("扩展", officialMaskExpansion, maybe("解析模型包含 expansion，但页面未验证渲染行为")),
  ...
  },
  {
    ...
    rows: [
      ...
      row("亮度遮罩", officialMattesLuma, maybe("未系统验证亮度遮罩")),
      row("亮度反相遮罩", officialMattesLuma, maybe("未系统验证亮度反相遮罩")),
    ],
  },
  {
    id: "mergePaths",
  ...
  },
  {
    ...
    rows: [
      ...
      row("描边", officialWebOnlySupported, maybe("尚未为 ADBE Stroke 建立页面级验证")),
      row("色调", officialWebOnlySupported, maybe("尚未为 Tint 建立页面级验证")),
      row("三色调", officialWebOnlySupported, maybe("尚未为 Tritone 建立页面级验证")),
      row("色阶（独立控制）", officialLayerEffectsLevels, maybe("尚未为 Levels 建立页面级验证")),
      row("高斯模糊", officialLayerEffectsBlur, maybe("尚未为 Gaussian Blur 建立页面级验证")),
      row("投影", officialLayerEffectsDropShadow, maybe("尚未为 Drop Shadow 建立页面级验证")),
    ],
  },
  {
    id: "text",
  ...
  },
  {
    ...
    rows: [
      ...
      row("时间拉伸", officialOtherTime, noWith("解析器尚未读取 layer.sr")),
      row("时间重映射", officialOtherTime, yes),
      row("标记", officialAllSupported, noWith("Animation 模型已预留 markers 字段，但解析器尚未读取 markers")),
    ],
  },
]
